...
PIPEWIDTHSIZE = 52 # the current PNG file size for the pipe image
...
# list of coins
COINS_LIST = (
    'assets/sprites/coin-yellow.png',
    'assets/sprites/coin-yellow.png',
)
...
def flappyGame():
    ...
    #IMAGES['calib'] = pygame.image.load('assets/sprites/twist1.jpg').convert_alpha()
    #IMAGES['twist2'] = pygame.image.load('assets/sprites/twist2.jpg').convert_alpha()
    #IMAGES['twist3'] = pygame.image.load('assets/sprites/twist3.jpg').convert_alpha()
    # base (ground) sprite
    ...
    while True:
        ...
        # hitmasks for coins
        HITMASKS['coin'] = (
            getHitmask(IMAGES['coin'][0]),
            getHitmask(IMAGES['coin'][1]),
        )
        ...
        while x == 0:
            ...
            for event in pygame.event.get():
                if (event.type == KEYDOWN and (event.key == K_SPACE or event.key == K_UP)):
                    x = 1
        '''
        #showPreCalibrationScreen()
        showCalibrationScreen()
        crashInfo = mainGame(movementInfo)
        showGameOverScreen(crashInfo)
        #ser.write('C')

def showPreCalibrationScreen():
    """Shows calibration animation of flappy bird"""
    global calib_finished
    calib_finished = True
...
def showCalibrationScreen():
    ...
    while True:
        ...
        SCREEN.blit(IMAGES['player'][playerIndex],
                    (playerx, playery + playerShmVals['val']))
        #SCREEN.blit(IMAGES['calib'], (messagex, messagey))
        #SCREEN.blit(IMAGES['twist2'], (messagex, messagey))
        #SCREEN.blit(IMAGES['twist3'], (messagex, messagey))
        SCREEN.blit(IMAGES['base'], (basex, BASEY))
...
def showWelcomeAnimation():
    ...
    while True:
        ...
        if j == 1 or j == 2:
            ...
            for event in pygame.event.get():
                if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):   #ESC key closes program
                    pygame.quit()
                    sys.exit()
                if (event.type == KEYDOWN and (event.key == K_SPACE or event.key == K_UP)):   #The space or up keys can be used to start the calibration process
